---
title: "SSO"
description: "This guide provides a step-by-step walkthrough of configuring Single Sign-On (SSO) for DefendStack Secrets. SSO allows users to authenticate with their existing identity provider (IdP) credentials, simplifying access and improving security. "
---
...
* **Information from your IdP:** Gather the following details from your IdP's configuration panel:
  ...
  * **Client ID:** A unique identifier for your DefendStack Secrets application registered with your IdP.

  * **Client Secret:** A secret key used to authenticate your DefendStack Secrets application with your IdP.

* **DefendStack Secrets Callback URL:** This URL will be provided by DefendStack Secrets during the configuration process. You'll need to configure this URL in your IdP as an "Allowed Callback URL" or "Redirect URI."

### Configuration Steps

1. **Navigate to the SSO Settings Page:**

   * Open your web browser and go to DefendStack Secrets application: `https://<app-url>/settings?tab=sso`
...
3. **Enter SSO Provider Details:**
   ...
   * A form will appear where you need to input the required information. Fill in the fields as follows:
     ...
     * **Callback URL:** DefendStack Secrets will provide you with the Callback URL. Copy this URL.
...
3. **Test the SSO Configuration:**
   ...
   * After creating the SSO provider, you should be able to test the configuration. DefendStack Secrets may provide a test link, or you can try logging out and then logging back in. You should be redirected to your IdP for authentication.
...
* **Callback URL mismatch:** A common issue is a mismatch between the Callback URL configured in DefendStack Secrets and the one configured in your IdP. Verify these URLs carefully.

* **IdP Configuration:** Review your IdP's documentation to ensure your DefendStack Secrets application is correctly configured on their end.

### Further Assistance

If you experience any difficulties during the SSO configuration process, please contact your DefendStack Secrets administrator or refer to the official DefendStack Secrets documentation for further assistance.